...
class LlavaLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if (
            ...
            # print('spi_module  >> ', self.spi_module.dtype)

            if bboxes is not None and sum([len(bbox) for bbox in bboxes]) > 0:
                # print(bboxes)
                # # print('before spi_module  >> ', mlvl_spi_features[0].dtype, bboxes[0].dtype)
                # print('bboxes   >>> ', mlvl_spi_features[0].shape, bboxes, len(bboxes), mlvl_spi_features[0].dtype)
                
                mlvl_spi_features = spi_module(mlvl_spi_features, bboxes)
            ...
            # print('input_ids   >>> ', input_ids)  # torch.Size([6, 1024])
            # print('input_ids   >>> ', input_ids.shape, image_features.shape)  # torch.Size([6, 1024])
            for cur_input_ids, cur_input_embeds, spi_feat in zip(input_ids, inputs_embeds, mlvl_spi_features):
                ...
                if vision_tower.config.use_im_start_end:
                    ...
                    # 包含每个conversation的image_start_tokens吗？

                    # 这个for循环能处理每个conversation的<image>吗？
                    for ist, image_start_token_pos in enumerate(image_start_tokens):
                        ...
                        if orig_embeds_params is not None:
                            cur_new_input_embeds = torch.cat(
                                (
                                    ...
                                    cur_image_features,  # 填充image_tokens
                                    cur_input_embeds[
                        ...
                        if spi_feat is not None:
                            ...
                            spi_mask = (cur_input_ids == bbox_token_id)
                            # print('spi_mask   >>> ', sum(spi_mask))
                            # print('spi_feat  >>> ', spi_feat.shape, spi_mask.sum(), spi_embeds[spi_mask].shape)
                            ...
                            try:
                                ...
                                # torch.Size([456])/([478])/([480])
                                # print('bbox_token_id  >>> ', bbox_token_id) # None
                                # print(type(cur_input_ids), type(bbox_token_id)) # Tensor, None
                                # assert (cur_input_ids == self.tokenizer.convert_tokens_to_ids(['<bbox>'])[0]).sum() == 0
                                assert bbox_token_id == None or (torch.sum(torch.tensor(cur_input_ids == bbox_token_id)) == 0)
                            except Exception as e:
                                print("An error occurred in llava_jack_gpt4roi.py line 300")
                                print(e)
                                pass
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    #         #     input_embeddings_avg = input_embeddings[:-num_new_tokens-1].mean(
    #         #         dim=0, keepdim=True)
    #         #     output_embeddings_avg = output_embeddings[:-num_new_tokens-1].mean(
    #         #         dim=0, keepdim=True)

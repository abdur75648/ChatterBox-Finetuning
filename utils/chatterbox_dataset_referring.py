...
class JackVanillaDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        with open(os.path.join(anno_path, 'CB_MRG.json')) as f:
...
class JackReferringDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.clip_image_processor = CLIPImageProcessor.from_pretrained(vision_tower)
        self.data_path = os.path.join(base_root)
        with open(os.path.join(anno_path, 'CB_REF.json')) as f:
            jack_json = json.load(f)
        self.jack_json = jack_json['data']
...
class RefCOCOsDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.clip_image_processor = CLIPImageProcessor.from_pretrained(vision_tower)
        self.data_path = os.path.join(base_root)
        with open(os.path.join(anno_path, 'CB-refcoco-REF.json')) as f:
            jack_json = json.load(f)
        self.jack_json = jack_json['data']
            ...
            if dataset == "jackvanilla":
                ...
                self.all_datasets.append(
                    RefCOCO(
                        ...
                        img_prefix="../datasets/MSCOCO2014/train2014/",
                        multimodal_cfg=multimodal_cfg,
                    )
                )
                ...
                self.all_datasets.append(
                    Flickr30k(
                        tokenizer=tokenizer,
                        ann_file='../OpenSource/final_flickr_separateGT_train.json',
                        img_prefix="../datasets/flickr30k/images/flickr30k-images/",
                        multimodal_cfg=multimodal_cfg,
                    )
                )
